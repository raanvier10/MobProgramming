...
class _AdminTransactionsPageState extends State<AdminTransactionsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _filter == null
        ? allTrx
        : allTrx.where((t) => t.status == _filter).toList();

    final totalLunas = allTrx
        .where((t) => context.read<AppState>().isTransactionFullyPaid(t))
        .fold(0.0, (s, t) => s + t.totalAmount);
    final totalPending =
        allTrx.where((t) => t.status == TransactionStatus.pending).length;
    final totalCicilan =
        allTrx.where((t) => t.status == TransactionStatus.cicilan).length;
                        ...
                        child: Column(
                            ...
                            children: [
                              ...
                              Row(children: [
                                _HeaderStat(
                                    'Lunas',
                                    CurrencyFormatter.formatCompact(totalLunas),
                                    AppColors.success500),
                                const SizedBox(width: 8),
                                _HeaderStat('Pending', '$totalPending',
                                    AppColors.accent400),
                                const SizedBox(width: 8),
                                _HeaderStat('Cicilan', '$totalCicilan',
                                    AppColors.info500),
                              ]),
                            ]))),
              ),
          ...
          SliverToBoxAdapter(
            child: Container(
              ...
              child: SingleChildScrollView(
                ...
                child: Row(children: [
                  _FilterChip('Semua', _filter == null,
                  ...
                  _FilterChip(
                      'Pending',
                      _filter == TransactionStatus.pending,
                      () =>
                          setState(() => _filter = TransactionStatus.pending)),
                  const SizedBox(width: 8),
                  _FilterChip(
                      'Cicilan',
                      _filter == TransactionStatus.cicilan,
                      () =>
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        // Cicilan progress
        if (transaction.status == TransactionStatus.cicilan &&
            transaction.totalTermin != null)
          Padding(
                  ...
                  children: [
                    Row(
                        ...
                        children: [
                          ...
                          Text(
                              '${transaction.paidTermin}/${transaction.totalTermin} termin',
                              style: AppTextStyles.labelSm.copyWith(
                                  color: AppColors.info700, fontSize: 10)),
                        ]),
                    const SizedBox(height: 4),
                    ClipRRect(
                        borderRadius: BorderRadius.circular(4),
                        child: LinearProgressIndicator(
                            value: (transaction.paidTermin ?? 0) /
                                (transaction.totalTermin ?? 1),
